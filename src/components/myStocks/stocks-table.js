...
class MyStocksTable extends Component {
  ...
  setTableContents = () => {
    axios
      .get("https://financial-portfolio-app.firebaseio.com/mystocks.json")
      .then(res => {
  ...
  stopTracking = event => {
    ...
    axios
      .delete(`https://financial-portfolio-app.firebaseio.com/mystocks.json/${id}.json`)
      .catch(err => {
        console.log(err);
        alert("Network issue check  console for error.");
      });

    axios.post(`https://financial-portfolio-app.firebaseio.com/tickers.json`, {
